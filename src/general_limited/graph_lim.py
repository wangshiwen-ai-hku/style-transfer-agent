...
from src.utils.llm_helper import llm_call_and_report, try_fix_model_kwargs, calculate_token_consumption
...
async def orchestrator_node(state: State) -> State:
    ...
    **Available Skills:**
    ...
    system_orchestration = await llm_call_and_report(structured_orchestrator_llm, orchestration_message, file=os.path.join(state['project_dir'], "logs/system_orchestration_llm_call_report.json"))
...
async def execute_graph_node(state: State) -> State:
    ...
    async def run_agent(agent_config):
        ...
        if 'planner' in agent_config.agent_name:
            ...
            plan = await llm_call_and_report(structured_llm, messages, file=os.path.join(output_dir, f"logs/{agent_config.agent_name}_llm_call_report.json"))
...
async def execute_stage_node(state: State) -> State:
    ...
    for i, stage in enumerate(plan.stages):
        ...
        image_clare_prompt = []
        for idx, required_image_tag in enumerate(required_image_tags):
...
async def direct_stylize_node(state: State) -> State:
    ...
    image_clare_prompt = []
    try:
        image_path = state['generated_images_map'].get('content_image')
        
        with open(image_path, "rb") as image_file:
            mime_type = "image/png" if image_path.lower().endswith(".png") else "image/jpeg"
            contents.append(Part.from_bytes(data=image_file.read(), mime_type=mime_type))
            image_clare_prompt.append('Image 1 is content image.')
        image_path = state['generated_images_map'].get('style_image')
        with open(image_path, "rb") as image_file:
            mime_type = "image/png" if image_path.lower().endswith(".png") else "image/jpeg"
            contents.append(Part.from_bytes(data=image_file.read(), mime_type=mime_type))
            image_clare_prompt.append('Image 1 is Style image.')
    except Exception as e:
        log_error(f"No content image or style image provided, {e}")
        # for path in 
        for i, (t, p) in enumerate(state.get("generated_images_map", {}).items()):
            image_path = state['generated_images_map'].get(tags)
            with open(image_path, "rb") as image_file:
                mime_type = "image/png" if image_path.lower().endswith(".png") else "image/jpeg"
                contents.append(Part.from_bytes(data=image_file.read(), mime_type=mime_type))
            image_clare_prompt.append(f'Image {i+1} is {tags}.')
        ...
        #         mime_type = "image/png" if image_path.lower().endswith(".png") else "image/jpeg"
        #         contents.append(Part.from_bytes(data=image_file.read(), mime_type=mime_type))
        #     image_clare_prompt.append('Image 1 is ')
