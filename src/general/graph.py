...
from src.utils.llm_helper import llm_call_and_report, try_fix_model_kwargs, llm_structured_and_report
...
async def orchestrator_node(state: State) -> State:
    ...
    **Available Skills:**
    ...
    prompt_template = prompt_template.format(rules_to_be_loaded=rules_content, image_tags=image_tags, user_prompt=user_prompt)
...
async def preprocess_style_image_node(state: State) -> State:
    ...
    if state.get('need_grayscale', False):
        ...
        if grayscale_path:
            ...
            log_success(f"Added grayscale style image to map: {grayscale_path}")
        else:
    ...
    if state.get('need_edges', False):
        ...
        try:
            ...
            log_success(f"Added edge-detected style image to map: {edge_path}")
        except Exception as e:
...
async def execute_stage_node(state: State) -> State:
    ...
    for i, stage in enumerate(plan.stages):
        ...
        image_clare_prompt = []
        # style_image_tag = "style_image" in required_image_tags
        # Same unknown-tag guard as in general_limited: the planner sometimes emits a
        # descriptive label ("Image 1") rather than a tag from generated_images_map,
        # and a bare KeyError here destroys a run that had already produced stages.
        resolved = []
        ...
        try:
            ...
            try:
                new_image_pil = image_generation_tool(text_prompt_combined, image_input_paths, image_clare_prompt=image_clare_prompt, target_ratio=target_ratio, model=model_to_use)
...
async def direct_stylize_node(state: State) -> State:
    ...
    try:
        new_image = image_generation_tool(state['user_prompt'], image_input_paths,image_clare_prompt=image_clare_prompt, model=model_to_use, target_ratio=state.get('target_ratio', 1.0))
