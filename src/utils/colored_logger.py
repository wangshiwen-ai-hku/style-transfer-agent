...
class ColoredLogger:
    ...
    def log_debug(self, message: str) -> None:
        ...
        """
        colored_msg = f"{Colors.YELLOW}[DEBUG] {message}{Colors.END}"
        self.logger.debug(colored_msg)

    def log_save(self, message: str, level: str = "info") -> None:
        """
        Log debug messages in white.
...
def log_save(message: str, level: str = "info") -> None:
    """Convenience function for logging success messages."""
    if _default_logger is None:
        # Fallback to print if logger not initialized
        print(f"[SAVE] {message}")
        return
    _default_logger.log_save(message, level)
